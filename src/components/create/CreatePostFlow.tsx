...
import { X, ChevronLeft, MapPin, Tag, Film, Image as ImageIcon, Video, Loader2 } from 'lucide-react';
...
export default function CreatePostFlow() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = postType === 'reel'
        ? { videoUrl: mediaUrls[0], caption }
        : { mediaUrls, caption, location, type: mediaUrls.length > 1 ? 'carousel' : 'photo' };
  ...
  const accept = postType === 'photo' ? 'image/*' : postType === 'reel' ? 'video/*' : 'image/*,video/*';
  ...
  return (
    <div className="fixed inset-0 bg-white dark:bg-black z-50 flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
        {step === 'type' ? (
          <button onClick={() => router.back()} className="text-gray-700 dark:text-gray-300"><X size={24} /></button>
        ) : (
          <button onClick={() => setStep(s => s === 'caption' ? 'media' : 'type')} className="text-gray-700 dark:text-gray-300">
            <ChevronLeft size={24} />
          </button>
        )}
        <h2 className="text-base font-semibold text-gray-900 dark:text-white">
          {step === 'type' ? 'Crear' : step === 'media' ? 'Seleccionar medios' : 'Nueva publicación'}
        </h2>
        {step === 'media' && mediaUrls.length > 0 ? (
          <button onClick={() => setStep('caption')} className="text-blue-500 font-semibold text-sm">
            Siguiente
          </button>
        ) : step === 'caption' ? (
          <button onClick={handleSubmit} disabled={isSubmitting} className="text-blue-500 font-semibold text-sm disabled:opacity-50 flex items-center gap-1">
      ...
      {step === 'type' && (
        <div className="flex-1 flex flex-col justify-center px-6 gap-4">
          <p className="text-center text-gray-500 dark:text-gray-400 text-sm mb-2">¿Qué quieres crear?</p>
          {[
            { type: 'photo' as PostType, icon: ImageIcon, label: 'Foto o carrusel', desc: 'Comparte imágenes con tu comunidad', color: 'from-blue-400 to-blue-600' },
            { type: 'video' as PostType, icon: Video, label: 'Video', desc: 'Comparte un video corto o largo', color: 'from-green-400 to-green-600' },
            { type: 'reel' as PostType, icon: Film, label: 'Reel', desc: 'Crea un video vertical en pantalla completa', color: 'from-purple-500 to-pink-500' },
          ].map(opt => (
            <button
              key={opt.type}
              onClick={() => { setPostType(opt.type); setStep('media'); }}
            ...
            >
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${opt.color} flex items-center justify-center flex-shrink-0`}>
            ...
            >
              <div className="flex flex-col items-center gap-4 p-8 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 dark:hover:bg-blue-950/20 transition-colors">
                <div className="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center">
                  {postType === 'photo' ? <ImageIcon size={32} className="text-gray-400" /> : <Film size={32} className="text-gray-400" />}
                </div>
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    {postType === 'photo' ? 'JPG, PNG, WebP • Hasta 10 archivos' :
                     postType === 'reel' ? 'MP4, MOV • Hasta 1GB' :
                     'Imágenes y videos'}
                  </p>
                </div>
      ...
      {step === 'caption' && (
        <div className="flex-1 overflow-y-auto">
          ...
          {[
            { icon: MapPin, label: 'Añadir ubicación', value: location, onChange: setLocation, placeholder: 'Ubicación...' },
          ].map(({ icon: Icon, label, value, onChange, placeholder }) => (
            <div key={label} className="flex items-center gap-3 px-4 py-4 border-t border-gray-100 dark:border-gray-800">
