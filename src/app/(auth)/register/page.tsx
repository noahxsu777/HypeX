...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black flex flex-col items-center justify-center px-6 py-10">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">Crea tu cuenta gratis</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <div>
            ...
            {errors.name && <p className="text-xs text-red-500 mt-1 px-1">{errors.name.message}</p>}
          </div>

          <div>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm">@</span>
              <input {...register('username')} placeholder="nombre_de_usuario" autoCapitalize="none"
                className="w-full pl-8 pr-4 py-3.5 bg-gray-100 dark:bg-gray-900 rounded-xl text-sm text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500" />
            </div>
            {errors.username && <p className="text-xs text-red-500 mt-1 px-1">{errors.username.message}</p>}
          </div>
          ...
          <div>
            <div className="relative">
              <input {...register('password')} type={showPwd ? 'text' : 'password'}
                placeholder="Contraseña (mín. 8 caracteres)" autoComplete="new-password"
            ...
            {pwd && (
              <div className="flex gap-1 mt-2 px-1">
                {[0,1,2,3].map(i => (
                  <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i < strength ? strengthColors[strength-1] : 'bg-gray-200 dark:bg-gray-700'}`} />
                ))}
              </div>
            )}
          ...
          <button type="submit" disabled={loading}
            className="w-full py-3.5 bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 text-white font-semibold rounded-xl text-sm flex items-center justify-center gap-2 disabled:opacity-60 mt-1">
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
          ¿Ya tienes cuenta?{' '}
          <Link href="/login" className="font-semibold text-purple-500">Inicia sesión</Link>
        </p>
      </div>
    </div>
  );
